...
/// Alignment of a value in the data blob, matching the reader's typed access.
fn align(value: &RGDValue) -> usize {
    match value {
        RGDValue::Float(_) | RGDValue::Int(_) | RGDValue::List(_) | RGDValue::List2(_) => 4,
        RGDValue::LocString(_) => 2,
        RGDValue::Boolean(_) | RGDValue::CString(_) => 1,
    }
}
...
fn write_list(nodes: &[RGDNode]) -> Vec<u8> {
    ...
    for (node, value) in sorted.iter().zip(&values) {
        let alignment = align(&node.value);
    ...
    for (node, &offset) in sorted.iter().zip(&offsets) {
        ...
        out.write_i32::<LittleEndian>(node.value.data_type().code()).unwrap();
...
fn write_value(value: &RGDValue) -> Vec<u8> {
    match value {
        RGDValue::Float(f) => f.to_le_bytes().to_vec(),
        RGDValue::Int(i) => i.to_le_bytes().to_vec(),
        RGDValue::Boolean(b) => vec![u8::from(*b)],
        RGDValue::CString(s) => {
            let mut v = s.as_bytes().to_vec();
            v.push(0);
            v
        }
        RGDValue::LocString(s) => {
            let mut v = Vec::new();
        ...
        RGDValue::List(children) | RGDValue::List2(children) => write_list(children),
    }
}
...
fn gather_keys(nodes: &[RGDNode]) -> Vec<(u64, String)> {
    ...
    fn walk(nodes: &[RGDNode], out: &mut Vec<(u64, String)>, seen: &mut HashSet<u64>) {
        for node in nodes {
            ...
            if let RGDValue::List(children) | RGDValue::List2(children) = &node.value {
